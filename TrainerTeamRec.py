...
def make_vector(team):
    vec = torch.zeros(len(mons))
    for mon in team:
        vec[mon.index] = 1
    return vec


def make_action(team, step, action):
    mon = team[step]
    names = [m.name for m in team if m.name != mon.name]
    indices = [m.index for m in team if m.name != mon.name]
    it = [i for i in range(len(mons)) if mons[i].name not in names]
    teams = []
    for i in range(len(it), max_actions):
        it.append(mon.index)
    actteam = [mons[i].copy() for i in indices]
    actteam.insert(step, mons[it].copy())
    return actteam
...
class TeamRecAgent():
    ...
    def learn(self):
        ...
        batch = self.random.choice(max_mem, self.batch_size, replace=False)
